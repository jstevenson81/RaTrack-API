...
export interface ICosmosClient<T extends IDocument> {
   ...
   * @param {string} docType This is the type of document we are trying to query
   * @returns {Promise<IDocument>}  This is the found item.
   */
  getOneAsync(id: string, docType: string): Promise<T>;
   ...
   */
  getAllAsync(docType: string): Promise<Array<T>>;
